...
public class PlayerHealthUI : MonoBehaviour
{
    ...
    {
        activeImageTransform.DOScale(0, _damageAnimationDuration).SetEase(_destroyAnimationEase).SetUpdate(true).SetLink(gameObject).OnComplete(() =>
        {
            activeImage.sprite = _playerUnhealthySprite;
            activeImageTransform.DOScale(1f, _damageAnimationDuration).SetEase(_recoverAnimationEase).SetUpdate(true).SetLink(gameObject);
        });
    }
