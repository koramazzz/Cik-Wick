namespace MaskTransitions
{
    ...
    public class TransitionManager : MonoBehaviour
    {
        ...
        [Tooltip("Runs transition using unscaled time so it works while game is paused")]
        ...
        void StartAnimation(float? totalTime = null)
        {
            ...
            maskRect.DOSizeDelta(new Vector2(maxSize, maxSize), animationTime).SetEase(Ease.InOutQuad).SetUpdate(useUnscaledTime);
            if (rotation)
                maskRect.DORotate(new Vector3(0, 0, 180), animationTime, RotateMode.FastBeyond360).SetEase(Ease.InOutQuad).SetUpdate(useUnscaledTime);
        }
        ...
        Tween StartAnimationForLoad(float? totalTime = null)
        {
            ...
            Tween blueTweenSize = maskRect.DOSizeDelta(new Vector2(maxSize, maxSize), animationTime).SetEase(Ease.InOutQuad).SetUpdate(useUnscaledTime);

            Sequence animationSequence = DOTween.Sequence().SetUpdate(useUnscaledTime).Join(blueTweenSize);

            if (rotation)
            {
                Tween blueTweenRotate = maskRect.DORotate(new Vector3(0, 0, 180), animationTime).SetEase(Ease.InOutQuad).SetUpdate(useUnscaledTime);
        ...
        void EndAnimation(float? totalTime = null)
        {
            ...
            parentMaskRect.DOSizeDelta(new Vector2(maxSize, maxSize), animationTime).SetEase(Ease.InOutQuad).SetUpdate(useUnscaledTime);
            if (rotation)
                parentMaskRect.DORotate(new Vector3(0, 0, 180), animationTime).SetEase(Ease.InOutQuad).SetUpdate(useUnscaledTime);
        }
        #endregion
        ...
        IEnumerator PlayTransitionWithDelay(float transitionTime, float startDelay)
        {
            ...
            yield return useUnscaledTime ? new WaitForSecondsRealtime(dividedTime) : new WaitForSeconds(dividedTime);
